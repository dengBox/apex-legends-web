import { Engine3D, Camera3D, ComponentBase, Vector3, Vector3Ex, clamp, PointerEvent3D } from '@orillusion/core'
...
export default class MouseController extends ComponentBase {
  /**
     * internal camera
     */
  private _camera: Camera3D | undefined
  /**
  ...
  public onUpdate () {
    ...
    const x = (this._position.x - this._cPosition.x) / step
    const y = (this._position.y - this._cPosition.y) / step
    const z = (this._position.z - this._cPosition.z) / step
  ...
  private onPointerDown (e: PointerEvent3D) {
    ...
    if (e.mouseCode === 2) { this._isPanning = true }
  }
  ...
  private updateCamera () {
    ...
    const pos = this._spherical.getCoords()
